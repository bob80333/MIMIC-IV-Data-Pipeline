...
def preproc_meds_imputation(module_path:str, adm_cohort_path:str, mapping:str) -> pd.DataFrame:
  
    adm = pd.read_csv(adm_cohort_path, usecols=['hadm_id', 'admittime'], parse_dates = ['admittime'])
    med = pd.read_csv(module_path, compression='gzip', parse_dates = ['starttime', 'stoptime']) #usecols=['subject_id', 'hadm_id', 'drug', 'starttime', 'stoptime','ndc','dose_val_rx'],
    med = med.merge(adm, left_on = 'hadm_id', right_on = 'hadm_id', how = 'inner')
    med['start_hours_from_admit'] = med['starttime'] - med['admittime']
    med['stop_hours_from_admit'] = med['stoptime'] - med['admittime']
    ...
    #select only rows where starttime is less than stoptime
    #med = med[med['starttime'] <= med['stoptime']]
...
def ndc_meds(med, mapping:str) -> pd.DataFrame:
    ...
    # Inner join the med dataset to the mapping information
...
def preproc_labs(dataset_path: str, version_path: str, cohort_path: str, time_col: str, anchor_col: str, dtypes: dict, usecols: list) -> pd.DataFrame:
    """Function for getting hospital observations pertaining to a pickled cohort. Function is structured to save memory when reading and transforming data."""
    ...
    chunksize = 10000000
    for chunk in tqdm(pd.read_csv(dataset_path, compression='gzip', usecols=usecols, dtype=dtypes, 
                                  parse_dates=[time_col], chunksize=chunksize)):
        chunk = chunk.dropna(subset=['valuenum'])
        chunk['valueuom'] = chunk['valueuom'].fillna(0)
        
        chunk = chunk[chunk['subject_id'].isin(cohort['subject_id'].unique())]
        chunk_na = chunk[chunk['hadm_id'].isna()]
        chunk = chunk[chunk['hadm_id'].notnull()]
        #COMMENTED BY ROHAN (IMPUTATION HERE)
        #chunk_na = impute_hadm_ids(chunk_na[['subject_id', 'hadm_id', 'itemid', 'charttime', 'valuenum', 'valueuom']].copy(), adm)
        #del chunk_na['hadm_id']
        #chunk_na = chunk_na.rename(columns={'hadm_id_new': 'hadm_id'})
        chunk_na=chunk_na[['subject_id','hadm_id','itemid','charttime','valuenum','valueuom']]
        chunk=chunk.append(chunk_na, ignore_index=True)
    ...
    df_cohort = pd.concat(chunks, ignore_index=True)
...
def preproc_labs_imputation(dataset_path: str, version_path: str, cohort_path: str, time_col: str, anchor_col: str, dtypes: dict, usecols: list) -> pd.DataFrame:
    """Function for getting hospital observations pertaining to a pickled cohort. Function is structured to save memory when reading and transforming data."""
    usecols = ['itemid','subject_id','hadm_id','charttime','valuenum','valueuom']
    dtypes = {
        'itemid':'int64',
    ...
    df_cohort=pd.DataFrame()
    cohort = pd.read_csv(cohort_path, compression='gzip', parse_dates = ['admittime'])
    d_labitems = pd.read_csv("/data/corpora_alpha/MIMIC/MIMIC_IV_2.2/files/mimiciv/2.2/hosp/d_labitems.csv.gz", compression='gzip')
    ...
    # Read the dataset in chunks
    chunksize = 10000000
    for chunk in tqdm(pd.read_csv(dataset_path, compression='gzip', 
                                  parse_dates=[time_col], chunksize=chunksize)):
        #chunk = chunk.dropna(subset=['valuenum'])
        #chunk['valueuom'] = chunk['valueuom'].fillna(0)
        ...
        chunk = chunk.merge(cohort[['hadm_id', 'admittime','dischtime']], how='inner', left_on='hadm_id', right_on='hadm_id')
        # Need to also add the details from d_labitems
        chunk = chunk.merge(d_labitems, how='inner', left_on='itemid', right_on='itemid')
        # I want to see how the columns are now
        #print(chunk.columns)
        chunk['charttime'] = pd.to_datetime(chunk['charttime'])
        chunk['lab_time_from_admit'] = chunk['charttime'] - chunk['admittime']
        #chunk = chunk.dropna()
...
def preproc_proc_imputation(dataset_path: str, cohort_path:str, time_col:str, anchor_col:str, dtypes: dict, usecols: list) -> pd.DataFrame:
    ...
    df_cohort = merge_module_cohort()
    d_icd = pd.read_csv("/data/corpora_alpha/MIMIC/MIMIC_IV_2.2/files/mimiciv/2.2/hosp/d_icd_procedures.csv.gz", compression='gzip')
    # I want merge using both icd_code and icd_version
    df_cohort = df_cohort.merge(d_icd[['icd_code', 'icd_version', 'long_title']], how='left', on=['icd_code', 'icd_version'])
    df_cohort['proc_time_from_admit'] = df_cohort['chartdate'] - df_cohort['admittime']
    #df_cohort=df_cohort.dropna()
    # Print unique counts and value_counts
    print("# Unique ICD9 Procedures:  ", df_cohort.loc[df_cohort.icd_version == 9].icd_code.dropna().nunique())
...
def preproc_icd_module_imputation(module_path:str, adm_cohort_path:str, icd_map_path=None, map_code_colname=None, only_icd10=True) -> pd.DataFrame:
    """Takes an module dataset with ICD codes and puts it in long_format, optionally mapping ICD-codes by a mapping table path"""    
    
    def get_module_cohort(module_path:str, cohort_path:str):
        module = pd.read_csv(module_path, compression='gzip', header=0)
        adm_cohort = pd.read_csv(adm_cohort_path, compression='gzip', header=0)
        return module.merge(adm_cohort[['hadm_id']], how='inner', left_on='hadm_id', right_on='hadm_id')
    ...
    def standardize_icd(mapping, df, root=False):
        ...
        if only_icd10:
            ...
            df['root'] = df[col_name].apply(lambda x: x[:3] if type(x) is str else np.nan)
    module = get_module_cohort(module_path, adm_cohort_path)
    #Add code to include the long title of the disease from the d_icd_diagnoses table
    d_icd = pd.read_csv("/data/corpora_alpha/MIMIC/MIMIC_IV_2.2/files/mimiciv/2.2/hosp/d_icd_diagnoses.csv.gz", compression='gzip')
    # I want merge using both icd_code and icd_version
    module = module.merge(d_icd[['icd_code', 'icd_version', 'long_title']], how='left', on=['icd_code', 'icd_version'])
