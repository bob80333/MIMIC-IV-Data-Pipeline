...
#         out[['subject_id', 'hadm_id', 'stay_id', 'itemid', 'charttime', 'intime', 'event_time_from_admit']].to_csv("./data/features/preproc_out_icu.csv.gz", compression='gzip', index=False)
    ...
    if med_flag:
        ...
        med = preproc_meds("/data/corpora_alpha/MIMIC/MIMIC_IV_2.2/files/"+version_path+"/hosp/prescriptions.csv.gz", './data/cohort/'+cohort_output+'.csv.gz','./utils/mappings/ndc_product.txt')
    ...
    if lab_flag:
        ...
        lab = preproc_labs("/data/corpora_alpha/MIMIC/MIMIC_IV_2.2/files/"+version_path+"/hosp/labevents.csv.gz", version_path,'./data/cohort/'+cohort_output+'.csv.gz','charttime', 'base_anchor_year', dtypes=None, usecols=None)
